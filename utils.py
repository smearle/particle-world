...
def infer(env, generator, archive, pg_width, pg_delay):
    ...
    return infer_elites(env, generator, elites, pg_width, pg_delay)


def infer_elites(env, generator, elites, pg_width, pg_delay):
    pygame.init()
    running = True
    while running:
        for g_weights in elites:
            _pg_anim(generator, g_weights, env, pg_width, pg_delay)
...
def _pg_anim(generator, model, env, pg_width, pg_delay):
    screen = pygame.display.set_mode([pg_width, pg_width])
    pg_scale = pg_width / env.width
    obj, bcs = simulate(generator, model, env, render=True, screen=screen, pg_delay=pg_delay, pg_scale=pg_scale)
    print(f'obj: {obj} \n bcs: {bcs}')
